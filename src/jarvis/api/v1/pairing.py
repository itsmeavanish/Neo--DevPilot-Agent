...
# Prefix /pairing is applied in v1/router.py — avoid /pairing/pairing/... duplication
...
async def phone_websocket(websocket: WebSocket, phone_id: str):
    ...
    try:
        ...
        # Keep verified pairing; only drop the live socket so brief disconnects recover
        sess = manager.get_session(phone_id)
        if sess:
            sess.websocket = None
